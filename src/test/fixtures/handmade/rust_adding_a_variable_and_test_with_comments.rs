...
fn painting() -> Result<()> {
    ...
    // minimal 0.307% -> 0.303%. The option is off under `FULL`, which this fix leaves byte-identical
    // at 0.307%, so `FULL` sets the limit now. Any earlier number in this comment that disagrees with
