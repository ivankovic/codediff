...
fn painting() -> Result<()> {
    ...
    // own text and its own place and shifted only because of an edit before it: minimal 1.491% ->
    // 1.352%. The option is off under `FULL`, which this fix leaves byte-identical at 1.119%, so
