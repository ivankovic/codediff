...
fn painting() -> Result<()> {
    ...
    // minimal 17.812% -> 4.375%. The option is off under `FULL`, which this fix leaves byte-identical
    // at 17.812%, so `FULL` sets the limit now. Any earlier number in this comment that disagrees with
...
fn invariants() -> Result<()> {
    assert_ground_truth_invariants("shellscript-ansible-ansible-a-small-add")
}
