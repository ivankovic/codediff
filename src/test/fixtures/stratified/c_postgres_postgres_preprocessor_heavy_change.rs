...
fn painting() -> Result<()> {
    ...
    // minimal 0.725% -> 0.207%. The option is off under `FULL`, which this fix leaves byte-identical
    // at 0.518%, so `FULL` sets the limit now. Any earlier number in this comment that disagrees with
...
fn invariants() -> Result<()> {
    assert_ground_truth_invariants("c-postgres-postgres-preprocessor-heavy-change")
}
