...
fn mapping() -> Result<()> {
    test::helper::human_mapping::assert_matches_human_mapping(
        "rust-gyulyvgc-sniffnet-rename-one-identifier",
    )
}

#[test]
fn painting() -> Result<()> {
    // measured 2026-09-11: minimal 0.007%, full 0.031%
...
fn invariants() -> Result<()> {
    ...
    // for a renamed identifier is not painted this way yet (`last_filtered_packet_timestamp` against `last_packet_timestamp`). Recorded as found; the
