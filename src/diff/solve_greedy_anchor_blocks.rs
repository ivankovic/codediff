...
* container-pairing heuristic (`solve_semantically_structural_nodes`, `solve_similar_flow_control`,
...
/// DICE_THRESHOLD`'s 2026-07-11 sweep. `{0.2, 0.35}` both landed at 829 mismatches (worse than the
/// old 0.5 default's 816 - too tight a filter rejects legitimate anchors, pushing more work onto
/// the final APTED pass), `0.5` (the old default) at 816, `{0.65}` at 813, and `{0.75, 0.8, 0.85}`
/// all tie at a **788** plateau (`0.95` regresses back to 797) - landed in the middle of that
/// plateau for the same "most margin from either regression cliff" reasoning `DICE_THRESHOLD` used.
/// Re-swept `DICE_THRESHOLD` jointly at `0.8` across `{0.8, 0.9, 0.95}`: all three tie at 788 too,
/// so the two thresholds don't meaningfully interact in this range - the gain is attributable to
/// `MAX_COST_RATIO` alone. Per-fixture: `c-cpython-autogenerated-code` improved 58->33 and
/// `cpp-laydbird-change-function-signature` improved 75->60; `c-postgres-real-logic-change`
/// regressed 8->20 (already flagged elsewhere in `TODO.md` as the corpus's largest single cost-gap
/// outlier, `algorithm_cost` 484 vs. `human_cost` 49 - a looser anchor threshold plausibly lets it
/// grab a cheaper-but-wronger reuse there). Net -28 (-3.4%), 2 improved vs. 1 regressed - kept.
