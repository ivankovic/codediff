...
## Design history moved out of source (2026-09-05)

The 2026-09-05 code health pass kept each of these passages' *design invariant* next to the code
and moved the chronology - what was tried, on which date, and which fixtures it broke - here. Each
source site points at this heading.

### `solve_greedy_anchor_blocks`: why the positional gate exists

An earlier version of this pass scored *every* candidate pair against every other (an all-pairs
B*A search) and regressed 9 `optimal_solutions` fixtures: `sequence_edit_cost` only looks at a
pair's own direct children, so nothing stopped it scoring two structurally *unrelated* nodes as
"cheap to match" whenever their children happened to hash-coincide (e.g. two different
`call_expression`s - one in a `for` loop condition, one in a `return` statement - whose
`argument_list` both happened to be `()`). Adding `nodes::is_block_container` narrowed the
candidate *kinds* enough to fix most of that, but one regression survived: a byte-identical
`statement_block` that the human mapping relocates into a newly-inserted `try_statement` wrapper
scored a *near-zero* cost ratio - the cheapest possible match by this heuristic's own reckoning.
The positional key answers both: the two unrelated `call_expression`s have unrelated ancestor
paths (`for_statement > binary_expression > ...` vs. `return_statement > pointer_expression >
...`), so they are never even compared; the relocated `statement_block`'s after-side path gains an
extra `try_statement` segment the before-side path lacks, so the pair is rejected regardless of
how cheap its content score is. Both problems and the design are also under "TRIED AND REVERTED
(2026-07-12)" and its "2026-07-12, positional anchor" follow-up in the root `TODO.md`.

### `solve_identical_diagnostic_statements`: no fixture coverage

Its placement after phase 1 and before phase 4 follows the same reasoning the since-deleted
(2026-08-14) `solve_similar_flow_control` documented for its own placement inside phase 4. It has
no effect on the `rust-turbopack-module-rule` fixture that motivated that deleted pass: its one
`bail!(...)` isn't byte-identical between before/after (the message text changed to list an added
"json" module type), so it wouldn't match here regardless. And even if it were identical, the
enclosing `match_expression` - `bail!` arm included - gets fully resolved via a real
`apted::for_nodes` call on the container (through phase 4's named-group matching on the enclosing
declaration), since `for_nodes` computes a complete edit script over the whole subtree, not just
the root pair. So the pass is validated only by its unit tests and has no real-fixture coverage.

### `myers.rs`: `flat_children`'s gate and the equal-count recursion

**The gate (`FLAT_MIN_CHILDREN` on the unmatched count).** Gating on total count instead was
tried and reverted (2026-08-14): a large, mostly-already-matched flat container (confirmed live
case: a Rust `mod` full of otherwise-identical `#[test]` functions, 259 direct children but only
~40 still unmatched) can fall below the unmatched threshold and fall through to general
tree-edit-distance over a much bigger *pruned residual forest* instead - real, and expensive
(18.5s of one fixture's 28s total for a container Myers would resolve in milliseconds) - but
switching the gate to total count also routed containers that used to correctly fall through to
general APTED into the `FLAT_UNMATCHED_RECURSE_LIMIT`-capped residual handling, which was strictly
weaker above that cap (atomic delete/insert, no partial-match search): broke 9 corpus fixtures
(`css-wordpress-wordpress-autogenerated-file` and others, `reason=APTED("final_pass")`
mismatches) and was reverted for lack of a fix that didn't weaken the residual path. Re-attempted
2026-08-16 (phases-4-7 rearchitecture) with the fix: the leftover recursion gated on an *entry
count*, which was the wrong signal for "is real APTED still affordable" - 90 tiny scattered edits
(small combined size, many entries) is cheap; 5 edits that each touch a huge subtree is not.
Adding `FLAT_UNMATCHED_RECURSE_MAX_TOTAL_SIZE` alongside it (the mechanism Phase 3b validated for
`resolve_residual_forest_via_myers_lcs`) separates those two cases.

**Equal-count leftover entries are recursed per position, never pooled, and uncapped.** The
module's doc comment once called those entries "genuine ordered siblings under one shared parent"
and assumed pooling them (up to `FLAT_UNMATCHED_RECURSE_LIMIT`) was safe. It isn't, even at small
scale: on `java-nextcloud-android-add-if-branch-with-reused-return` a mere 2-before/2-after pool
still let APTED invent a plausible-but-wrong cross-match between two unrelated methods, deleting
one wholesale (54 -> 343 mismatches) - the same risk `kotlin-refactor-function` (Phase 3b) and
`tsx-excalidraw-...-huge-file` (the `flat_children` fix) showed for larger pools. A size cap on
the per-position pairs (matching `FLAT_UNMATCHED_RECURSE_MAX_TOTAL_SIZE`) was tried and cost real
fixtures for no benefit: `java-nextcloud-android-add-two-function-calls` (2 entries each side,
5,454 combined nodes) regressed until the cap was removed, and removing it made no fixture worse
and did not move latency (p99/max within noise).

### `residual.rs`: the residual forest's per-position recursion

Pooling the residual's maximal-unmatched-roots was caught as unsafe on `kotlin-refactor-function`
(2026-08-15): an unrelated deleted function's parameter/return-expression nodes got matched, via
reason `fast_fallback`, to look-alike nodes elsewhere in the same gap, flipping the fixture from 0
to 32 mismatches. Equal counts (2026-08-16) are therefore recursed individually at their fixed
document-order position - the original exactly-one-entry case generalized from "one pair" to "N
independently-recursed pairs". The per-position pairs are uncapped in size, the same as
`resolve_flat_tree_pair`'s equal-count branch: the since-deleted `RESIDUAL_SEGMENT_MAX_TOTAL_SIZE`
bounded a *pool's* cost, never a correctness risk here, and measurement on the sibling function
found no fixture worse and no latency movement once removed.
