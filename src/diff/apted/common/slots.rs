...
/// itself is matched wrongly.** Where a container is matched one level off, its delimiters follow
/// it and are wrong for the same reason it is. That is the honest arrangement: a delimiter that
/// contradicts its own parent is not a better diff, it is two defects cancelling. Only moving a
/// delimiter whose opposite number already agrees leaves the underlying defect live whenever the
/// DP gets *both* ends wrong at once.
...
/// at the cost of not enforcing that check across the move itself. Without the filter the check
/// fires on exactly that mechanism - repeated, reordered `case` bodies pruned by
/// `greedy_anchor_block` - and forbids valid pairings around them.
...
mod reclaim_tests {
    ...
    fn a_surviving_call_keeps_its_own_closing_paren_when_a_nested_call_is_removed() {
        // `f(a, g())` -> `f(a, b)`. Two `)` before, one after, and matching either costs the
        // same, so nothing but this rule stops the DP from keeping the one belonging to the
        // *removed* `g()` and deleting the outer call's.
