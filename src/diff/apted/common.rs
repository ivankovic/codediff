...
///
/// Gating on total count instead of unmatched count was tried and reverted once already
/// (2026-08-14): a large, mostly-already-matched flat container (confirmed live case: a Rust `mod`
/// full of otherwise-identical `#[test]` functions, 259 direct children but only ~40 still
/// unmatched) can fall below this threshold and fall through to general tree-edit-distance over a
/// much bigger *pruned residual forest* instead - real, and expensive (18.5s of one fixture's 28s
/// total for a container Myers would resolve in milliseconds) - but switching the gate to total
/// count also routed containers that used to correctly fall through to general APTED into this
/// path's `FLAT_UNMATCHED_RECURSE_LIMIT`-capped residual handling instead, which was strictly
...
/// mismatches) before the full test suite caught it, and was reverted for lack of a fix that
/// didn't weaken the residual path's accuracy for containers that still needed general APTED.
///
/// Re-attempted 2026-08-16 (phases-4-7 rearchitecture, `TODO.md`) now that fix exists:
/// `resolve_flat_tree_pair`'s leftover recursion gates on `FLAT_UNMATCHED_RECURSE_LIMIT` - an
/// *entry count* - which was the wrong signal for "is real APTED still affordable on this
/// residual." Tree-edit-distance cost is driven by total node count, not entry count: 90 tiny
/// scattered edits (small combined size, many entries) is cheap; 5 edits that each touch a huge
/// subtree (large combined size, few entries) is not - the entry-count-only cap let the second
/// case through (unbounded) while blocking the first (bounded, but denied anyway). Adding a total-
/// size cap alongside it (`FLAT_UNMATCHED_RECURSE_MAX_TOTAL_SIZE`, same mechanism Phase 3b already
/// validated for `resolve_residual_forest_via_myers_lcs`'s own leftover recursion) separates those
/// two cases correctly, closing the gap the 2026-08-14 attempt couldn't - see that constant's doc
/// comment for the corpus measurement.
fn flat_children(root_id: usize, meta: &ASTMetadata) -> Option<Vec<usize>> {
...
const FLAT_UNMATCHED_RECURSE_MAX_TOTAL_SIZE: usize = 2000;
