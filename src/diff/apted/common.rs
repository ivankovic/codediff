...
///
/// Gating on total count instead of unmatched count was tried and reverted (2026-08-14): a large,
/// mostly-already-matched flat container (confirmed live case: a Rust `mod` full of otherwise-
/// identical `#[test]` functions, 259 direct children but only ~40 still unmatched) can fall below
/// this threshold and fall through to general tree-edit-distance over a much bigger *pruned
/// residual forest* instead - real, and expensive (18.5s of one fixture's 28s total for a
/// container Myers would resolve in milliseconds) - but switching the gate to total count also
/// routes containers that used to correctly fall through to general APTED into this path's
/// `FLAT_UNMATCHED_RECURSE_LIMIT`-capped residual handling instead, which is strictly weaker above
/// that cap (atomic delete/insert, no partial-match search): broke 9 corpus fixtures
/// (`css-wordpress-wordpress-autogenerated-file` and others, `reason=APTED("final_pass")`
/// mismatches) before the full test suite caught it. Fixing the gate-miss case needs a change that
/// doesn't weaken the residual path's accuracy for containers that still need general APTED - not
/// yet found.
fn flat_children(
    root_id: usize,
    meta: &ASTMetadata,
