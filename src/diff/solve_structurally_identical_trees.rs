...
* reference node (largest subtrees first) it looks for an unclaimed after-side node whose subtree
* has the same shape - same node kinds in the same arrangement - even if leaf values differ. On a
* hit it maps the pair with the StructurallyIdenticalSubtrees reason, then descends the child
* subtrees, mapping them with the StructurallyIdenticalAncestor reason. Each paired node's text is
* compared to pick the operation: Identical when the values match, Update (cost 1) when they
* differ.
*
* The traversal itself is shared with `solve_identical_trees` - see `hash_tree_matching::solve`;
* this file only configures it for structural hashes.
*/
