...
The `TODO.md` and `SPECS.md` files this document names were retired on 2026-09-26; working notes
now live in an untracked log, and design rationale in the code's module docs.
...
## 2026-09-25 file-level comments

All twelve comments of 2026-09-25 are done: `coverage` depends on `test`, and `test-rust` is the
Rust suite alone, next to `test-python`; README.pdf, results.txt, RULES_AND_PREFERENCES.md,
HANDOVER.md, `process_gentoo_package_list.sh` and `view_test_diff.sh` are deleted (README.pdf is
ignored so it cannot come back); the tiny repository list points at github.com; the contribution
notice is bold; `main.rs` opens with the copyright header; `dataset.sh` lost its "why this became a
flag" history. The README screenshot lives in `assets/` and is generated by `make readme-screenshot`
the way the GIF is: `tui::screenshot` draws the viewer with the TUI's own widgets into an offscreen
terminal and `scripts/render_tui_screenshot.py` rasterizes it.
